...
function( X , Y , alpha=0.05 , print.results=TRUE){
  ...
  if (print.results) {
    ...
    cat("************  10% acceptance criterion analogous to T2EQ and f2 \t*******"         , "\n\n")
    cat("*******************************************************************************"   , "\n\n")
  }
  ...
  #equivalence margin  
  D_ems       <- matrix(10,d,1)                                         # Vector for calculating equivalence margin analogically to
  eq_margin   <- sum( (D_ems * D_ems) / (0.5 * (vars_X + vars_Y) ) )    # T2EQ approach (see Suarez et al. (2020))
